...
void InitModule(ModuleStruct *Mod)
{
f (! StrLen(Mod->Path)) Mod->Path=CopyStr(Mod->Path,DEFAULT_HOSTS_PATH);
Mod->Data=STREAMOpenFile(Mod->Path,O_RDONLY);

if (! Mod->Data) LogToFile(Settings.LogFilePath,"ERROR: Unable to open hosts file %s",Mod->Path);
...
int QueryHostsFile(ModuleStruct *LS, char *SearchName,DNSMessageStruct *Query)
{
